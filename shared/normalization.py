...
import numpy as np
from shared.constants import INITIAL_BALANCE, MAX_POSITION_BTC,DEFAULT_INDICATORS
...
def normalize_state(balance, price_vec, tech_vec, turbulence_vec, signal_vec, holdings,
                    ...
                    enable_turbulence=True, enable_vix=True):
    """
    ...
    if any(asset.get('enabled', False) and asset.get('col_name') == 'vix' for asset in DEFAULT_INDICATORS) and idx < len(turbulence_vec):
...
def inverse_normalize_state(
    *,
    ...
    if any(asset.get('enabled', False) and asset.get('col_name') == 'vix' for asset in DEFAULT_INDICATORS):
